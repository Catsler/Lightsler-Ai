...
```javascript
export async function translateUrlHandle(handle, targetLang, context = {}) {
  // 智能判断是否需要翻译
  if (shouldKeepOriginalHandle(handle, context)) {
    return handle;
  }
  ...
  const translatedSegments = await Promise.all(
    segments.map(seg => translateSegment(seg, targetLang))
  );
  
  // 生成SEO友好的URL
  return generateSeoFriendlyUrl(translatedSegments, targetLang);
}
```
...
1. **SEO优化的URL生成**
```javascript
function generateSeoFriendlyUrl(segments, lang) {
  const cleanSegments = segments
    .map(seg => seg.toLowerCase())
    .map(seg => removeSpecialCharacters(seg))
    .map(seg => transliterate(seg, lang))
    .filter(seg => seg.length > 0);
  
  // 限制URL长度
  const url = cleanSegments.join('-');
  if (url.length > 50) {
...
function transliterate(text, lang) {
  ...
  if (lang === 'zh-CN' || lang === 'ja' || lang === 'ko') {
    return pinyinize(text); // 使用拼音或罗马字
  }
...
2. **智能跳过规则**
```javascript
function shouldKeepOriginalHandle(handle, context) {
  // 保留产品型号
  if (/^[A-Z0-9]{3,}-\d+$/.test(handle)) {
    return true;
  }
  
  // 保留已优化的URL
  if (context.previouslyOptimized) {
    return true;
  }
  
  // 保留品牌相关URL
  if (BRAND_WORDS.some(brand => 
    handle.toLowerCase().includes(brand.toLowerCase())
  )) {
    return true;
  }
