...
function shouldTranslateThemeField(key, value) {
  ...
  if (THEME_TECHNICAL_PATTERNS.some(pattern => pattern.test(key))) {
    console.log(`[Theme翻译] 跳过技术字段: ${key}`);
    return false;
  }
  
  // 3. 跳过Liquid模板变量和标签
  if (value.includes('{{') || value.includes('{%')) {
    console.log(`[Theme翻译] 跳过Liquid模板: ${key}`);
    return false;
  }
  
  // 4. 跳过纯数字或特殊格式
  if (/^\d+$/.test(value) || /^#[0-9A-Fa-f]{3,6}$/.test(value)) {
    console.log(`[Theme翻译] 跳过数字/颜色值: ${key}`);
    return false;
  }
  
  // 5. 跳过URL（但不跳过包含URL的文本）
  if (/^https?:\/\//.test(value) && !value.includes(' ')) {
    console.log(`[Theme翻译] 跳过纯URL: ${key}`);
    return false;
  }
  ...
  if (isTranslatable) {
    console.log(`[Theme翻译] 需要翻译字段: ${key}`);
    return true;
  }
  ...
  if (placeholderTexts.includes(value)) {
    console.log(`[Theme翻译] 检测到占位符文本，需要翻译: ${key} = "${value}"`);
    return true;
  }
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (shouldTranslateThemeField(fullKey, value)) {
      try {
        ...
        console.log(`[Theme翻译] 成功翻译 ${fullKey}: "${value.substring(0, 50)}..." -> "${translatedText.substring(0, 50)}..."`);
      } catch (error) {
        console.error(`[Theme翻译] 翻译失败 ${fullKey}:`, error.message);
...
export async function translateThemeResource(resource, targetLang) {
  console.log(`[Theme翻译] 开始翻译Theme资源: ${resource.resourceType} - ${resource.id}`);
  ...
  if (!contentFields || Object.keys(contentFields).length === 0) {
    console.error(`[Theme翻译] 错误：Theme资源缺少contentFields数据: ${resource.id}`);
  ...
  switch (resourceType) {
    ...
    case 'ONLINE_STORE_THEME_SETTINGS_DATA_SECTIONS':
      console.log(`[Theme翻译] 处理JSON模板类型资源`);
      
      // 处理themeData字段（JSON内容）
      if (contentFields.themeData) {
        try {
          console.log(`[Theme翻译] 解析并翻译themeData`);
          ...
          console.log(`[Theme翻译] themeData翻译完成`);
        } catch (error) {
          console.error('[Theme翻译] 处理themeData失败:', error);
          // 保留原始数据
      ...
      if (contentFields.dynamicFields) {
        console.log(`[Theme翻译] 处理dynamicFields，共${Object.keys(contentFields.dynamicFields).length}个字段`);
        ...
        for (const [key, fieldData] of Object.entries(contentFields.dynamicFields)) {
          ...
          if (!shouldTranslate && resourceType === 'ONLINE_STORE_THEME_JSON_TEMPLATE') {
            ...
            if (typeof fieldValue === 'string' && fieldValue.length >= 2 && /[a-zA-Z]/.test(fieldValue)) {
              ...
              if (!isTechnical && !isBrand && !fieldValue.includes('{{') && !fieldValue.includes('{%')) {
                console.log(`[Theme翻译] JSON Template特殊处理，强制翻译: ${key} = "${fieldValue}"`);
          ...
          if (shouldTranslate) {
            try {
              ...
              console.error(`[Theme翻译] 翻译dynamicField失败 ${key}:`, error.message);
        ...
        for (const field of contentFields.translatableFields) {
          if (shouldTranslateThemeField(field.key, field.value)) {
            try {
              ...
              console.error(`[Theme翻译] 翻译field失败 ${field.key}:`, error.message);
    ...
    case 'ONLINE_STORE_THEME_LOCALE_CONTENT':
      console.log(`[Theme翻译] 处理本地化内容类型资源`);
      ...
      if (contentFields.localeContent) {
        try {
          ...
          console.error('[Theme翻译] 翻译localeContent失败:', error);
    ...
    default:
      console.log(`[Theme翻译] 未知的Theme资源类型: ${resourceType}，使用通用处理`);
  ...
  console.log(`[Theme翻译] Theme资源翻译完成: ${resource.id}`);
  return result;
}
...
export function validateThemeTranslation(original, translated) {
  ...
  if (typeof original !== typeof translated) {
    console.error('[Theme翻译] 验证失败：数据类型不一致');
    return false;
  }
  ...
  if (typeof original === 'object' && original !== null) {
    ...
    if (originalKeys.length !== translatedKeys.length) {
      console.error('[Theme翻译] 验证失败：键数量不一致');
      return false;
    }
    ...
    for (const key of originalKeys) {
      if (!(key in translated)) {
        console.error(`[Theme翻译] 验证失败：缺少键 ${key}`);
        return false;
      }
    }
  }
