 ...
 * 清理过期的webhook事件
 */
const handleCleanupError = createServiceErrorHandler('WEBHOOK_CLEANUP', {
  throwErrors: false,
...
async function cleanupWebhookEventsInternal() {
...
export const cleanupWebhookEvents = handleCleanupError(cleanupWebhookEventsInternal);

/**
 * 获取webhook事件统计
 */
const handleWebhookStatsError = createServiceErrorHandler('WEBHOOK_CLEANUP');

async function getWebhookEventStatsInternal() {
  const now = new Date();
...
export const getWebhookEventStats = handleWebhookStatsError(getWebhookEventStatsInternal);

/**
 * 重试失败的webhook事件
 */
const handleWebhookRetryError = createServiceErrorHandler('WEBHOOK_CLEANUP');

async function retryFailedWebhookEventsInternal(limit = 10) {
...
export const retryFailedWebhookEvents = handleWebhookRetryError(retryFailedWebhookEventsInternal);
