 ...
 * 获取同步状态处理函数
 */
async function handleGetSyncStatus({ session }) {
 ...
 * 执行同步操作处理函数
 */
async function handleSyncAction({ request, admin, session }) {
 ...
 * GET请求：获取同步状态
 */
export const loader = createApiRoute(handleGetSyncStatus, {
  requireAuth: true,
 ...
 * POST请求：执行同步操作
 */
export const action = createApiRoute(handleSyncAction, {
  requireAuth: true,
  operationName: '执行同步操作'
});