...
async function handleGetStatus({ session, params }) {
  ...
  const untranslatedResources = normalizedResources.filter((r) => !r.hasTranslationForLanguage);
  const translatedResources = normalizedResources.length - untranslatedResources.length;

  const resourceSummary = {
    total: normalizedResources.length,
    translated: translatedResources,
    untranslated: untranslatedResources.length,
    untranslatedResourceIds: untranslatedResources.slice(0, 50).map((r) => r.id)
  };
...
// GET请求：获取总体状态
export const loader = createApiRoute(handleGetStatus, {
  requireAuth: true,
  operationName: '获取状态'
});
 ...
 * POST请求处理函数：查询特定任务状态
 */
async function handlePostStatus({ session, params }) {
  const jobId = params.jobId;
  const resourceId = params.resourceId;
  const queryType = params.type || "job"; // job, resource, queue
...
// POST请求：查询特定任务状态
export const action = createApiRoute(handlePostStatus, {
  requireAuth: true,
