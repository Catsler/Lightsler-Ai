...
# Fynony应用权限更新脚本
# 用途：在本地部署更新Fynony生产应用的Shopify权限配置

set -e  # 遇到错误立即退出

echo "🚀 开始更新 Fynony 应用权限..."
echo ""
...
# 检查配置文件存在
if [ ! -f "阿里云轻量服务器部署文件/shop1-shopify.app.toml" ]; then
    echo "❌ 错误：找不到Fynony配置文件"
    exit 1
fi
...
sed -i.bak 's/^SHOPIFY_API_KEY=.*/SHOPIFY_API_KEY=f97170933cde079c914f7df7e90cd806/' .env
echo "✅ 已替换为Fynony配置（shopify.app.toml + .env）"
echo ""
echo "📋 配置信息："
echo "   - Client ID: f97170933cde079c914f7df7e90cd806"
echo "   - 应用名: Lightsler AI - Fynony"
...
echo ""
echo "2️⃣  找到 'Lightsler AI - Fynony' 应用"
echo ""
echo "3️⃣  点击 'Test on development store'"
echo ""
echo "4️⃣  选择 'sshvdt-ai' 店铺"
echo ""
